...
taudit ships 17 built-in rules covering common CI/CD authority patterns. Custom rules let you encode organisation-specific policies on top of the same authority graph — without modifying taudit itself.

Custom rules were added in **v0.4.0** and are loaded via the `--rules-dir` flag.

## Overview

Custom rules are YAML files placed in a directory. Each file defines a single rule. When you pass `--rules-dir ./rules/` to `taudit scan`, taudit:

1. Reads every `*.yml` and `*.yaml` file in the directory (sorted, for deterministic output).
2. Parses each file into a rule definition.
3. Runs the built-in propagation engine to generate all authority paths in the graph.
4. Evaluates your custom rules against those paths — the same paths the built-in rules use.
5. Emits findings alongside built-in findings in whatever output format you chose.

Custom rule findings are indistinguishable from built-in findings in SARIF, JSON, and terminal output — except that the message includes the rule id in brackets (e.g., `[my_rule_id] Rule name: source -> sink`).

Custom rules complement the built-in rules; they do not replace them. Both run on every `taudit scan` invocation.
