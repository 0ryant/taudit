...
| **V1-6** | **Re-publishable contract docs** — [`authority-graph.md`](authority-graph.md), [`positioning.md`](positioning.md), ADRs, **golden paths**, product [`CHANGELOG.md`](../CHANGELOG.md). Optional: dedicated graph-schema changelog file. | S | **In progress** |

When **V1-2**, **V1-5**, and **V1-6** reach “done” to the quality bar above, the v1.0 charter is effectively complete for downstream stack work (tsign / axiom).
