# Interactive Debug Action In Authority Workflow

**Rule ID:** `interactive_debug_action_in_authority_workflow`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::interactive_debug_action_in_authority_workflow` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn interactive_debug_action_in_authority_workflow`)
