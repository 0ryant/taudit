# Sensitive Value In Job Output

**Rule ID:** `sensitive_value_in_job_output`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::sensitive_value_in_job_output` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn sensitive_value_in_job_output`)
