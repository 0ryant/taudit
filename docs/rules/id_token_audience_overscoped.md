# Id Token Audience Overscoped

**Rule ID:** `id_token_audience_overscoped`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::id_token_audience_overscoped` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn id_token_audience_overscoped`)
