# Child Pipeline Trigger Inherits Authority

**Rule ID:** `child_pipeline_trigger_inherits_authority`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::child_pipeline_trigger_inherits_authority` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn child_pipeline_trigger_inherits_authority`)
