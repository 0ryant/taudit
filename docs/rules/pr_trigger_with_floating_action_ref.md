# Pr Trigger With Floating Action Ref

**Rule ID:** `pr_trigger_with_floating_action_ref`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::pr_trigger_with_floating_action_ref` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn pr_trigger_with_floating_action_ref`)
