# Dotenv Artifact Flows To Privileged Deployment

**Rule ID:** `dotenv_artifact_flows_to_privileged_deployment`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::dotenv_artifact_flows_to_privileged_deployment` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn dotenv_artifact_flows_to_privileged_deployment`)
