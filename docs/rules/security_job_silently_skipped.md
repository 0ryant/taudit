# Security Job Silently Skipped

**Rule ID:** `security_job_silently_skipped`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::security_job_silently_skipped` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn security_job_silently_skipped`)
