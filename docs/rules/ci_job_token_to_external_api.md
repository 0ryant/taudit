# Ci Job Token To External Api

**Rule ID:** `ci_job_token_to_external_api`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::ci_job_token_to_external_api` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn ci_job_token_to_external_api`)
