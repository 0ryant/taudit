# Gh Cli With Default Token Escalating

**Rule ID:** `gh_cli_with_default_token_escalating`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::gh_cli_with_default_token_escalating` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn gh_cli_with_default_token_escalating`)
