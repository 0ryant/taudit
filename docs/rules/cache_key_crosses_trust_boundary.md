# Cache Key Crosses Trust Boundary

**Rule ID:** `cache_key_crosses_trust_boundary`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::cache_key_crosses_trust_boundary` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn cache_key_crosses_trust_boundary`)
