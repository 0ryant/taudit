# Ci Token Triggers Downstream With Variable Passthrough

**Rule ID:** `ci_token_triggers_downstream_with_variable_passthrough`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::ci_token_triggers_downstream_with_variable_passthrough` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn ci_token_triggers_downstream_with_variable_passthrough`)
