# Pr Specific Cache Key In Default Branch Consumer

**Rule ID:** `pr_specific_cache_key_in_default_branch_consumer`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::pr_specific_cache_key_in_default_branch_consumer` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn pr_specific_cache_key_in_default_branch_consumer`)
