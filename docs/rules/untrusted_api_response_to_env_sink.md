# Untrusted Api Response To Env Sink

**Rule ID:** `untrusted_api_response_to_env_sink`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::untrusted_api_response_to_env_sink` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn untrusted_api_response_to_env_sink`)
