# Secrets Inherit Overscoped Passthrough

**Rule ID:** `secrets_inherit_overscoped_passthrough`
...
The rule fires when its detection logic in `crates/taudit-core/src/rules.rs::secrets_inherit_overscoped_passthrough` returns a non-empty `Vec<Finding>`. The detection is deterministic: it inspects parsed graph metadata (no path traversal). See the function's doc-comment in source for the precise signal it watches.
...
- [`crates/taudit-core/src/rules.rs`](../../crates/taudit-core/src/rules.rs) — detection implementation (`pub fn secrets_inherit_overscoped_passthrough`)
