//! Generated tool `taudit_baseline_init`.
...
pub struct TauditBaselineInitArgs {
    pub paths: String,
    #[serde(default)]
...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        let schema = schemars::schema_for!(TauditBaselineInitArgs);
        ToolDefinition {
            name: "taudit_baseline_init".into(),
            title: Some("Initialise baseline".into()),
            description: "Snapshot CURRENT findings on the given pipelines into `.taudit/baselines/<pipeline-content-hash>.json`.".into(),
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        let args: TauditBaselineInitArgs = match serde_json::from_value(arguments) {
            Ok(args) => args,
