...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        let schema = schemars::schema_for!(TauditExplainArgs);
        ToolDefinition {
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        let args: TauditExplainArgs = match serde_json::from_value(arguments) {
            Ok(args) => args,
