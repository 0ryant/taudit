...
impl McpTool for Tool {
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        ...
        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(concat!("../../.mcpact/tools/taudit_baseline_promote.json"))) {
            Ok(spec) => spec,
        ...
        plan.argv.push(match args.expires_at { Some(v) => v, None => String::new() });
        }
        ...
        plan.argv.push(match args.severity_override { Some(v) => v, None => String::new() });
        }
        ...
        match Executor::default().execute(plan).await {
            Ok(result) => {
                let event = mcpact_audit::EvidenceEvent::tool_executed("taudit_baseline_promote", &plan_for_audit, &result);
                let sink = server_config::audit_sink();
                let _ = sink.emit(&event).await;
