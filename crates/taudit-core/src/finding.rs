...
mod fingerprint_tests {
    ...
    #[test]
    fn helper_authority_rule_ids_stay_backcompat_stable() {
        let cases = [
            (
        ...
        for (category, expected) in cases {
            let finding = make_finding(category, "helper authority finding", vec![]);
            assert_eq!(
                rule_id_for(&finding),
                expected,
                "helper authority rule id changed; update the L4-10 migration plan first"
            );
        }
    }
