...
- **Workflow linters in governance CI** — pinned **actionlint** + **yamllint** via **[`scripts/install-ci-linters.sh`](scripts/install-ci-linters.sh)** and **[`.yamllint`](.yamllint)**; **[`scripts/quality-gate.sh`](scripts/quality-gate.sh)** `ci-governance` runs them before **taudit**. Wired from [`.github/workflows/quality.yml`](.github/workflows/quality.yml), [`azure-pipelines.yml`](azure-pipelines.yml), and [`.gitlab-ci.yml`](.gitlab-ci.yml).
...
- **GitHub Actions hygiene** — [`release.yml`](.github/workflows/release.yml) macOS Intel matrix uses **`macos-15-intel`** (was deprecated **`macos-13`**); [**`mutation-coverage.yml`**](.github/workflows/mutation-coverage.yml) / [**`taudit-pr-diff.yml`**](.github/workflows/taudit-pr-diff.yml) shell steps satisfy **shellcheck** SC2129; [**`stack-integration.yml`**](.github/workflows/stack-integration.yml) notice lines wrapped for **yamllint** line-length.
